...
var open = exports.open = function(path, options) {
	...
	var stream = new Stream(read ? fs.createReadStream(file) : fs.createWriteStream(file, Boolean(options.append)));
	if (options.binary) {
		return stream;
...
Path.prototype.join = function() {
	return new Path(join.apply(null, [this.toString()].concat(Array.prototype.slice.call(arguments))));
};
...
Path.prototype.resolve = function() {
	return new Path(resolve.apply(null, [this.toString()].concat(Array.prototype.slice.call(arguments))));
};
...
['absolute', 'base', 'canonical', 'directory', 'normal', 'relative'].forEach(function(name) {
	Path.prototype[name] = (function(name) {
		return function() {
			return new Path(exports[name].apply(this, [this.toString()].concat(Array.prototype.slice.call(arguments))));
		};
	})(name);
});
...
[
	...
	'open', 'read', 'remove', 'removeTree', 'rename', 'size', 'split', 'symbolicLink',
	'touch', 'write'
	...
	Path.prototype[name] = (function(name) {
		return function() {
			...
			var result = exports[name].apply(this, [this.toString()].concat(Array.prototype.slice.call(arguments)));
			if (result === undefined)
				result = this;
			return result;
		};
