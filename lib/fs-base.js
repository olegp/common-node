/**
 * @fileoverview File and path related functionality as defined in <a
 *               href="http://wiki.commonjs.org/wiki/Filesystem/A">CommonJS
 *               Filesystem/A</a> .
 */
require('fibers');
var p = require('path');
...
/*
 * 'absolute', 'base', 'copy', 'copyTree', 'directory', 'extension', 'join',
 * 'makeTree', 'listDirectoryTree', 'listTree', 'normal', 'open', 'path',
 * 'Path', 'read', 'relative', 'removeTree', 'resolve', 'write', 'split', //
 * fs-base 'canonical', 'changeWorkingDirectory', 'workingDirectory', 'exists',
 * 'isDirectory', 'isFile', 'isReadable', 'isWritable', 'list', 'makeDirectory',
 * 'move', 'lastModified', 'openRaw', 'remove', 'removeDirectory', 'size',
 * 'touch',
 * 
 * 'symbolicLink', 'hardLink', 'readLink',
 * 
 * 'isLink', 'same', 'sameFilesystem',
 * 
 * 'iterate',
 * 
 * 
 * 'Permissions', 'owner', 'group', 'changePermissions', 'changeOwner',
 * 'changeGroup', 'permissions'
 */
...
var fss = {};
var methods = ['lstat', 'readdir', 'rmdir', 'unlink', 'rename', 'mkdir',
		'symlink', 'link', 'readlink', 'chmod'];
 ...
 * @throws Error
 *           if path is not a file or could not be removed.
 */
var remove = exports.remove = function(path) {
	var file = resolveFile(path);
	// TODO check if we need to remove dirs as well
	fss.unlink(file);
};
 ...
 *          the directory path
 * @throws Error
 *           if the file or directory could not be removed.
 */
...
var isReadable = exports.isReadable = function(path) {
	// var file = resolveFile(path);
	// fss.stat(file).mode
	// TODO check against current uid gid
};
...
var isFile = exports.isFile = function(path) {
	...
	// TODO check what happens to symlinks
	return fss.stat(file).isFile();
};
...
var isDirectory = exports.isDirectory = function(path) {
	var file = resolveFile(path);
	// TODO file doesn't exist
	return fss.lstat(file).isSymbolicLink();
};
...
var symbolicLink = exports.symbolicLink = function(source, target) {
	// TODO resolveFile(path)
	fss.symlink(source, target);
};
...
function iterate(path) {
	...
	// spec requires iterator(), native iterators/generators only have
	// __iterator__().
 ...
 * 
 * @param {number|object}
 *          permissions a number or object representing the permissions.
 * @param constructor
 */
...
Permissions.prototype.update = function(permissions) {
	...
	var users = ['owner', 'group', 'other'], permissions = ['read', 'write',
			'execute'];
...
var permissions = exports.permissions = function(path) {
	// TODO resolveFile
	return new Permissions(fss.stat(path).mode & 0777);
};

var owner = exports.owner = function(path) {
	try {
		// TODO resolveFile?
		return fss.stat(path).uid;
	} catch(error) {
		return null;
	}
};
