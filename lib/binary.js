...
var ByteArray = exports.ByteArray = function(contentOrLength, charsetOrClear) {
	...
	// ByteArray(length) - create ByteArray of specified size
		...
		this._buffer = new Buffer(arguments[0]);
		this._buffer.fill(0);
	}
...
var EMPTY_STRING = new ByteString(new Buffer(0));
...
ByteArray.prototype.shift = function() {
	if (this.length > 0) {
		...
		this.length--;
		var buffer = this._cow ? new Buffer(this.length) : this._buffer;
...
function slice() {
	...
	return Buffer.prototype.slice.apply(this.buffer.slice(0, this._length), args);
}
...
ByteArray.prototype.slice = function(begin, end) {
	return new ByteArray(slice.apply(this, arguments));
};
...
function split(delimiters, options) {
	...
	var length = this._length, buffer = this._buffer;
...
ByteArray.prototype.split = function(delimiter, options) {
	return split.apply(this, arguments).map(function(component) {
...
Binary.prototype.toArray = function(charset) {
	if (charset) {
		var value = this._buffer.toString(encodings[charset], 0, this._length);
...
Binary.prototype.decodeToString = function(charset) {
	return this._buffer.toString(encodings[charset], 0, this._length);
};
...
Binary.prototype.indexOf = function(byteValue, start, stop) {
	...
	return Array.prototype.indexOf.apply(buffer, arguments);
};
...
Binary.prototype.lastIndexOf = function(byteValue, start, stop) {
	...
	if (arguments.length > 1) {
		buffer = Buffer.prototype.slice.apply(buffer, Array.prototype.slice.call(arguments, 1));
	}
...
ByteString.prototype.slice = function(begin, end) {
	return new ByteString(slice.apply(this, arguments));
};
