...
function slice(that, args) {
	args = Array.prototype.map.call(args, function(arg) {
		return typeof arg === 'number' ? arg : 0;
	});
	return Buffer.prototype.slice.apply(that.buffer.slice(0, that._length), args);
}
...
ByteArray.prototype.slice = function(begin, end) {
	return new ByteArray(slice(this, arguments));
};

function concat(that, args) {
	var components = [that], totalLength = that._length;

	Array.prototype.forEach.call(args, function(argument) {
...
ByteString.prototype.slice = function(begin, end) {
	return new ByteString(slice(this, arguments));
};
