/**
 * @fileoverview Binary, ByteArray and ByteString classes as defined in <a
...
for( var i = 0; i < 128; i++)
	zeroArray.push(0);
var zeroBuffer = new Buffer(zeroArray);

/**
 * Zeroes the provided buffer with offset 'start' by setting all bytes to 0.
 * 
 * @param {Buffer}
 *          buffer
 * @start {Number} offset
 */
var zero = function(buffer, start) {
	return;
	var bufferLength = buffer.length, zeroLength = zeroBuffer.length;
	for( var i = (start || 0); i < bufferLength; i += zeroLength) {
		zeroBuffer.copy(buffer, i, 0, zeroLength);
	}
};
 ...
 * 
 * @param {Binary|Array|String|Number|Buffer}
 *          contentOrLength content or length of the ByteArray.
 * @param {String}
 *          [charset] the encoding name if the first argument is a String.
 * @constructor
 */
var ByteArray = exports.ByteArray = function() {
	...
	// string encoded with charset
		...
		console.log(arguments[0] instanceof Number);
 ...
 * href="http://nodejs.org/docs/v0.4.8/api/buffers.html#buffers">Node.js Buffer</a>.
 * 
 * @param {Binary|Array|String|Buffer}
 *          content the content of the ByteString.
 * @param {String}
 *          charset the encoding name if the first argument is a String.
 * @constructor
 */
 ...
 * 
 * @param {String}
 *          charset the name of the string encoding. Defaults to 'UTF-8'
 ...
 * 
 * @param {Function}
 *          comparator the function to compare entries
 ...
 * @param {Object}
 *          thisObj optional this-object for callback
 */
 ...
 * 
 * @param {Function}
 *          callback the function to call with each element of the ByteArray
 * @param initialValue
 *          optional argument to be used as the first argument to the first call
 *          to the callback
...
ByteArray.prototype.pop = function() {
	...
	// TODO sort this out, very expensive
	this.length--;
 ...
 * 
 * @param {Number}
 *          num... one or more numbers to append
 ...
 * 
 * @param {Number}
 *          index the index at which to start changing the ByteArray
 * @param {Number}
 *          howMany The number of elements to remove at the given position
 * @param {Number}
 *          elements... the new elements to add at the given position
 */
 ...
 *          index, begin indicates an offset from the end of the sequence.
 * @param {Number}
 *          end Zero-based index at which to end extraction. slice extracts up
 *          to but not including end. As a negative index, end indicates an
 *          offset from the end of the sequence. If end is omitted, slice
 ...
 * 
 * @param {Binary|Array}
 *          arg... one or more elements to concatenate
 * @returns {ByteArray} a new ByteArray
 ...
 * 
 * @param {Number|Binary}
 *          sequence the number or binary to look for
 * @param {Number}
 *          start optional index position at which to start searching
 * @param {Number}
 *          stop optional index position at which to stop searching
 ...
 * 
 * @param {Number|Binary}
 *          delimiter one or more delimiter items
 * @param {Object}
 *          options optional object parameter with the following optional
 *          properties:
 *          <ul>
 ...
 * @param {String}
 *          charset optional the name of the string encoding
 * @function
 */
 ...
 * @param {String}
 *          charset the name of the string encoding
 * @function
 */
