...
var ByteArray = exports.ByteArray = function(contentOrLength, charsetOrClear) {
	...
	// ByteArray(length) - create ByteArray of specified size
		...
		this.buffer = newBuffer(arguments[0]);
...
Object.defineProperty(ByteArray.prototype, 'length', {
	...
	set:function(length) {
		if (this._length < length) {
			...
			this.buffer.copy(buffer, 0, 0, this._length);
			buffer.fill(0, this._length);
			this.buffer = buffer;
		}
		this._length = length;
	}
...
ByteArray.prototype.map = function(callback, thisObject) {
	...
	for (var i = 0, length = this.length; i < length; i++) {
		result.buffer[i] = callback.call(thisObject, this.get(i), i, this);
	}
	return result;
};
...
ByteArray.prototype.unshift = function(num) {
	var length = arguments.length;
	var buffer = newBuffer(this.length + length);
	this.buffer.copy(buffer, length, 0, this.length);
...
ByteArray.prototype.splice = function(index, howMany, elements) {
	...
	var buffer = newBuffer(newLength);
	this.buffer.copy(buffer, index + inject.length, end);
...
ByteArray.prototype.copy = function(start, end, target, targetOffset) {
	var buffer = newBuffer(target.length);
	target.buffer.copy(buffer, 0, 0, target.length);
	this.buffer.copy(buffer, targetOffset, start, end);
...
function concat() {
	...
	components.forEach(function(component) {
		component.buffer.copy(buffer, offset);
...
ByteArray.prototype.concat = function(arg) {
	return new ByteArray(concat.apply(this, arguments));
};
