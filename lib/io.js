/**
 * @fileoverview Stream and TextStream classes as per
 * <a href="http://wiki.commonjs.org/wiki/IO/A">CommonJS IO/A</a>
 * as well as a ByteArray based in memory MemoryStream.
 */
 ...
 * @constructor
 * @param stream a <a href="http://nodejs.org/docs/v0.4.8/api/streams.html#streams">Node.js Stream</a> that can be readable, writeable or both
 */
 ...
 * @function
 */
Stream.prototype.writeable = function () {
    return this.stream && this.stream.writeable;
}
...
Stream.prototype.forEach = function (callback, thisObject) {
  ...
  if(!this.closed()) {
    ...
    while (data = yield()) {
        if (! (data instanceof Buffer)) break;
...
Stream.prototype.read = function (n) {
  ...
  if (n) {
    ...
    return ByteString.prototype.concat.apply(new ByteString(), 
      blocks.map(function(e) { return new ByteString(e); }));    
 ...
 * @constructor
 */
var MemoryStream = exports.MemoryStream = function MemoryStream(bufferOrCapacity) {
    var buffer, length;
    if (!bufferOrCapacity) {
