...
var Stream = exports.Stream = function(stream) {
	...
	this.drained = true;
	this.ended = false;
	if (this.readable() && stream !== process.stdout && stream !== process.stderr) {
		var that = this;
		...
		stream.on('readable', handleRead).on('end', function() {
			that.ended = true;
			handleRead();
		});
	}
...
Stream.prototype.readable = function() {
	return this.stream instanceof NodeStream.Readable || this.stream instanceof NodeStream.Duplex;
};
...
Stream.prototype.writable = function() {
	return this.stream instanceof NodeStream.Writable || this.stream instanceof NodeStream.Duplex;
};
...
Stream.prototype.read = function(n) {
	...
	return data ? new ByteString(data) : new ByteString();
};
...
Stream.prototype.write = function(source, begin, end) {
	...
	this.drained = false;
	var that = this;
	that.stream.write(source.buffer, function() {
		that.drained = true;
		if (that.flushing) {
...
Stream.prototype.copy = function(output) {
	this.stream.pipe(output.stream);
};
