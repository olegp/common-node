...
var MemoryStream = exports.MemoryStream = function(bufferOrCapacity) {
	...
	stream.read = function(num) {
		...
		if (isFinite(num)) {
			...
			result = buffer.slice(position, end).toByteString();
			position = end;
			return result;
		} else {
			result = buffer.slice(position, length).toByteString();
			position = length;
		}
...
var TextStream = exports.TextStream = function(io, options) {
	...
	this.write = function() {
		Array.prototype.forEach.call(arguments, function(data) {
			io.write(String(data).toByteString());
		});
		return this;
	};
