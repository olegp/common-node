/**
 * @fileoverview HTTP Client as defined in 
 * <a href="http://wiki.commonjs.org/wiki/HTTP_Client/A">CommonJS HTTP Client/A</a>.
 */
require('fibers');
var protocols = {
  http: require('http'),
...
/**
 * Set the body, headers, method, or url, or any combination thereof in 
 * the settings object. Attribute validity is enforced.
 ...
 * That is, if the user sets "content-type", and then later sets "Content-Type", then the first setting is lost.
 ...
 * This method return an error if a body was provided earlier via settings
 * that does not implement push.
...
HttpClient.prototype.write = function (data) {
  ...
  if(!this.guts.body.push) throw new Error("body does not have push()");
 ...
 * Calling read() does not block the application until the request is completed, 
 * but it does open the input stream such that the data can be read.
 */
