/**
 * @fileoverview HTTP Client as defined in <a
 *               href="http://wiki.commonjs.org/wiki/HTTP_Client/A">CommonJS
 *               HTTP Client/A</a>.
 */
require('fibers');
var protocols = {
 ...
 * 
 * @param key
 *          one of body, headers, method or url
 * @param val
 *          the value to set
 */
 ...
 * This method return an error if a body was provided earlier via settings that
...
HttpClient.prototype.read = function() {
	...
	req.socket.setTimeout(this.guts.timeout || 0);
	if(this.guts.timeout) {
		req.socket.addListener('timeout', function() {
			req.socket.destroy();
			fiber.run(new Error('Connection timeout'));
		});
	}
	req.addListener('error', function(exception) {
		fiber.run(exception);
	});
