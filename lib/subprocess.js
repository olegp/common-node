...
//TODO replace ringo/urils/arrays
var arrays = require('ringo/utils/arrays');

function parseArguments(args) {
	// arguments may end with a {dir: "", env: {}} object
	var opts = (args.length > 1 && arrays.peek(args) instanceof Object) ? Array
			.pop(args) : {};
 ...
 * command are optional: * `command` a string or array of strings containing the
 * command to execute * `dir` the directory to run the process in * `env`
 * alternative environment variables. If null the process inherits the
 * environment of the current process. * `binary` a boolean flag that uses raw
 * binary streams instead of text streams * `encoding` the character encoding to
 * use for text streams
 * 
 * @param {Object}
 *          args an object containing the process command and options.
...
var createProcess = exports.createProcess = function(args) {
	...
	var command = args.command, env = args.env, dir = args.dir, binary = args.binary, encoding = args.encoding;
	dir = dir ? new java.io.File(dir) : null;
	if(env && !Array.isArray(env)) {
		// convert env to an array of the form ["key=value", ...]
		env = [];
		for( var key in env) {
			env.push(key + "=" + env[key]);
		}
	} else if(!env) {
		env = null;
	}
	...
	return {
		 ...
		 * 
		 * @param {Stream}
		 *          input output stream to connect to the process's input stream
		 * @param {Stream}
		 *          output input stream to connect to the process's output stream
		 * @param {Stream}
		 *          errput input stream to connect to the process's error stream
 ...
 * 
 * @param {String}
 *          command... command and optional arguments as single or multiple
 *          string parameters
 * @param {Object}
 *          [options] options object. This may contain a `dir` string property
 *          specifying the directory to run the process in and a `env` object
