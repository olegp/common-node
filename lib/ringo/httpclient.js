...
function Headers(headers) {
	...
	Object.defineProperty(headers, "contains", {
		value : function(key) {
			return Boolean(this[key] !== undefined || (key = keys[key.toLowerCase()])
					&& this[key] !== undefined);
		}
	});
...
var extractOptionalArguments = function(args) {
	...
	if (args.length == 1) {
		...
		if (types[1] == 'function' && types[2] == 'function') {
			...
			throw new Error(
					'three argument form must be (url, success, error) or (url, data, success)');
		}
	}
